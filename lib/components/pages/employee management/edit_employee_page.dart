...
class _EditEmployeePageState extends State<EditEmployeePage> {
  ...
  void initState() {
    ...
    _loadDropdownData().then((_) {
      // Initialize form fields with existing data after dropdowns are loaded
  ...
  // Add this new method to initialize all form fields
  void _initializeFormFields() {
    setState(() {
      // Basic Info
      _selectedTitle = widget.employeeData['title'] ?? 'Mr';
      _empNameController.text = widget.employeeData['empName'] ?? '';
      _empCodeController.text = widget.employeeData['empCode'] ?? '';
      _emailController.text = widget.employeeData['email'] ?? '';
      
      // Set dropdown values only if they exist in the loaded lists
      _selectedGrade = _grades.contains(widget.employeeData['grade']) 
          ? widget.employeeData['grade'] 
          : null;
      ...
      _selectedBranch = _branches.contains(widget.employeeData['branch']) 
          ...
          : null;
      _selectedDepartment = _departments.contains(widget.employeeData['department']) 
          ? widget.employeeData['department'] 
          : null;
      _selectedDesignation = _designations.contains(widget.employeeData['designation']) 
          ? widget.employeeData['designation'] 
          : null;
      _selectedStatus = _statuses.contains(widget.employeeData['status']) 
          ? widget.employeeData['status'] 
          : null;
      
      // Check permission visibility after setting department and designation
      _checkPermissionVisibility();
      if (_showPermissionField && widget.employeeData['permission'] != null) {
        _selectedPermission = widget.employeeData['permission'];
      }
      
      // Dates
      _birthDateController.text = widget.employeeData['birthDate'] ?? '';
      _joinDateController.text = widget.employeeData['joinDate'] ?? '';
      _lastWorkingDateController.text = widget.employeeData['lastWorkingDate'] ?? '';
      _resignOfferDateController.text = widget.employeeData['resignOfferDate'] ?? '';
      _lastIncrementDateController.text = widget.employeeData['lastIncrementDate'] ?? '';
      _probationDateController.text = widget.employeeData['probationDate'] ?? '';
      _serviceMonthController.text = widget.employeeData['serviceMonth'] ?? '';
      
      // Payment
      _selectedPaymentMode = _paymentModes.contains(widget.employeeData['paymentMode']) 
          ? widget.employeeData['paymentMode'] 
          : null;
      
      // Bank Details
      final bankDetails = widget.employeeData['bankDetails'] as Map<String, dynamic>?;
      ...
      _bankBranchController.text = bankDetails?['branch'] ?? '';
      
      // Documents
      _aadharController.text = widget.employeeData['aadharNumber'] ?? '';
      _panController.text = widget.employeeData['panNumber'] ?? '';
    });
  }
  ...
  Future<void> _saveEmployee() async {
    ...
    try {
      ...
      // Update existing employee data to Firestore
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          Clipboard.setData(
                            ClipboardData(text: _generatePassword()),
                          );
                          ScaffoldMessenger.of(context).showSnackBar(
