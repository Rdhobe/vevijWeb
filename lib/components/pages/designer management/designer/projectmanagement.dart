...
import 'docvault.dart';
import 'hardwaresetup.dart';
...
class _ProjectManagementPageState extends State<ProjectManagementPage> {
                ...
                ElevatedButton.icon(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => const HardwaresetupPage(),
                      ),
                    );
                  },
      ...
      builder: (context, groupSnapshot) {
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, projectSnapshot) {
            ...
            if (groups.isEmpty && ungroupedProjects.isEmpty) {
              return const Center(
                child: Column(
              ...
              Row(
                children: [
                  ...
                  PopupMenuButton<String>(
                    onSelected: (value) =>
                        _handleGroupMenuAction(value, group.id, groupName),
                    itemBuilder: (context) => [
                      const PopupMenuItem(
                        value: 'edit',
                        child: Row(
  ...
  void _showGroupProjectsDialog(
    DocumentSnapshot group,
    List<DocumentSnapshot> groupProjects,
  ) {
    final groupData = group.data() as Map<String, dynamic>;
    final groupName = groupData['name'] ?? 'Unnamed Group';
  ...
  Widget _buildProjectCard(
    ...
    return GestureDetector(
      ...
      child: Container(
        margin: EdgeInsets.only(bottom: 15, left: isSubProject ? 20 : 0),
        decoration: BoxDecoration(
          color: isSubProject ? Colors.grey[50] : Colors.white,
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: isSetupComplete
                ? (isSubProject ? Colors.grey[300]! : Colors.grey[200]!)
                : Colors.orange.shade300,
            width: isSetupComplete ? 1 : 2,
          ),
                ...
                if (groupNameController.text.trim().isNotEmpty) {
                  ...
                  if (mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Project group created successfully!'),
                      ),
                    );
                  }
                }
                    ...
                    Expanded(
                      child: StreamBuilder<QuerySnapshot>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          final availableProjects = snapshot.data!.docs.where((
                            doc,
                          ) {
                          ...
                          final filteredProjects = availableProjects.where((
                            doc,
                          ) {
                            final data = doc.data() as Map<String, dynamic>;
  ...
  void _handleGroupMenuAction(String action, String groupId, String groupName) {
    switch (action) {
      case 'edit':
  ...
  void _showDeleteGroupConfirmation(String groupId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Are you sure? Projects in this group will become ungrouped.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
  ...
  void _deleteGroup(String groupId) async {
    try {
      ...
      batch.delete(
        FirebaseFirestore.instance.collection('projectGroups').doc(groupId),
      );
                      ...
                      onTap: () async {
                        ...
                        if (mounted) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                '$projectName added to ${groupData['name']}',
                              ),
                            ),
                          );
                        }
            ...
            ElevatedButton(
              onPressed: () {
                ...
                FirebaseFirestore.instance
                    .collection('projects')
                    .doc(projectId)
                    .update({'name': projectNameController.text});
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
